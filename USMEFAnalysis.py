...
from rpy2.robjects import numpy2ri, default_converter, ListVector
...
class USMEFAnalysis:
    ...
    def load_and_clean_data(self, start_date="2019-01-01", end_date="2025-12-31"):
        ...
        # --- Convert units ---
        df["hourly_emissions_mlb"] = (
            df["hourly_emissions"] * 2204.6 / 1_000_000
        )  # metric tons → million lbs
        df["hourly_generation_mkwh"] = (
            df["hourly_generation"] * 1000 / 1_000_000
        )  # GWh → million kWh
    ...
    def create_time_variables(self):
        ...
        df.loc[winter_mask, "yearseason"] = (
            "Winter_"
            + (df.loc[winter_mask, "year"] - 1).astype(str)
            + "_"
            + df.loc[winter_mask, "year"].astype(str)
        )

        winter_mask_end = df["month"] == 12
        df.loc[winter_mask_end, "yearseason"] = (
            "Winter_"
            + df.loc[winter_mask_end, "year"].astype(str)
            + "_"
            + (df.loc[winter_mask_end, "year"] + 1).astype(str)
        )
    ...
    def seasonal_adjustment_iteraction(self):
        ...
        df["hourly_generation_renewables_res"] = (
            model_gen_r.resid + model_gen_r.params["Intercept"]
        )
        ...
        df["hourly_generation_nonrenewables_res"] = (
            model_gen_nr.resid + model_gen_nr.params["Intercept"]
        )
        ...
        print(
            f"Non-Renewable Generation Coefficients: {len(model_gen_nr.params)} terms"
        )
        ...
        overall = pd.DataFrame(
            {
                ...
                "Std Dev": [
                    df["hourly_emissions"].std(),
                    df["hourly_generation"].std(),
                ],
    ...
    def plot_time_series(self):
        ...
        axes[0].set_title("Hourly Emissions", fontsize=18)
    ...
    def elegant_plot(
        self, x, y, title, ylabel, color="#1a1a1a", lw=0.7, save_path=None
    ):
        ...
        if save_path:
            ...
            fname = f"{save_path}/{title.replace(' ', '_').replace(':', '')}.svg"
    ...
    def plot_annual_diagnostics_dynamic(self, lags=100, save_path="images"):
        ...
        for year in years:
            ...
            model_gen = ols(
                "hourly_generation ~ factorinterval * factormonth + factordow + trend",
                data=df_year,
            ).fit()
            df_year["hourly_generation_res"] = model_gen.resid + model_gen.params[0]

            model_em = ols(
                "hourly_emissions ~ factorinterval * factormonth + factordow + trend",
                data=df_year,
            ).fit()
            ...
            ax1.plot(
                df_year["date_hourx"],
                df_year["hourly_generation_res"] / 1000,
                color="black",
                linewidth=0.8,
            )
            ...
            if save_path:
                fig1.savefig(
                    f"{save_path}/Generation_Res_{year}.pdf", bbox_inches="tight"
                )
            if save_path:
                fig1.savefig(
                    f"{save_path}/Generation_Res_{year}.svg", bbox_inches="tight"
                )
            plt.show()
            ...
            plot_acf(
                df_year["hourly_generation_res"].dropna(),
                lags=lags,
                ax=ax2,
                color="#1f77b4",
                zero=False,
                title=f"ACF Generation ({year})",
            )
            ...
            if save_path:
                fig2.savefig(
                    f"{save_path}/Generation_ACF_{year}.pdf", bbox_inches="tight"
                )
            if save_path:
                fig2.savefig(
                    f"{save_path}/Generation_ACF_{year}.svg", bbox_inches="tight"
                )
            plt.show()
            ...
            if save_path:
                fig3.savefig(
                    f"{save_path}/Generation_PACF_{year}.pdf", bbox_inches="tight"
                )
            if save_path:
                fig3.savefig(
                    f"{save_path}/Generation_PACF_{year}.svg", bbox_inches="tight"
                )
            plt.show()
            ...
            ax4.plot(
                df_year["date_hourx"],
                df_year["hourly_emissions_res"] / 1000,
                color="black",
                linewidth=0.8,
            )
            ...
            if save_path:
                fig4.savefig(
                    f"{save_path}/Emissions_Res_{year}.pdf", bbox_inches="tight"
                )
            if save_path:
                fig4.savefig(
                    f"{save_path}/Emissions_Res_{year}.svg", bbox_inches="tight"
                )
            plt.show()
            ...
            plot_acf(
                df_year["hourly_emissions_res"].dropna(),
                lags=lags,
                ax=ax5,
                color="#1f77b4",
                zero=False,
                title=f"ACF Emissions ({year})",
            )
            ...
            if save_path:
                fig5.savefig(
                    f"{save_path}/Emissions_ACF_{year}.pdf", bbox_inches="tight"
                )
            if save_path:
                fig5.savefig(
                    f"{save_path}/Emissions_ACF_{year}.svg", bbox_inches="tight"
                )
            plt.show()
            ...
            if save_path:
                fig6.savefig(
                    f"{save_path}/Emissions_PACF_{year}.pdf", bbox_inches="tight"
                )
            if save_path:
                fig6.savefig(
                    f"{save_path}/Emissions_PACF_{year}.svg", bbox_inches="tight"
                )
            plt.show()
    ...
    def plot_seasonal_diagnostics_dynamic(self, lags=100, save_path=None):
        ...
        for season in seasons:
            ...
            self.elegant_plot(
                df_season["date_hourx"],
                df_season["hourly_generation_res"],
            ...
            plot_acf(
                df_season["hourly_generation_res"].dropna(),
                lags=lags,
                ax=ax[0],
                color="#1f77b4",
                zero=False,
            ...
            for i, title in enumerate(["ACF – Hourly Load", "PACF – Hourly Load"]):
                ax[i].set_title(
                    f"{title} ({season})", loc="left", pad=10, weight="bold"
                )
            ...
            if save_path:
                fig.savefig(
                    f"{save_path}/ACF_PACF_Load_{season}.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                fig.savefig(
                    f"{save_path}/ACF_PACF_Load_{season}.svg",
                    dpi=300,
                    bbox_inches="tight",
                )
            plt.show()
            ...
            self.elegant_plot(
                df_season["date_hourx"],
                df_season["hourly_emissions_res"],
            ...
            fig, ax = plt.subplots(2, 1, figsize=self.get_figsize(10 * 2))
            plot_acf(
                df_season["hourly_emissions_res"].dropna(),
                lags=lags,
                ax=ax[0],
                color="#1f77b4",
                zero=False,
            ...
            for i, title in enumerate(
                ["ACF – Hourly Emissions", "PACF – Hourly Emissions"]
            ):
                ax[i].set_title(
                    f"{title} ({season})", loc="left", pad=10, weight="bold"
                )
            ...
            if save_path:
                fig.savefig(
                    f"{save_path}/ACF_PACF_Emissions_{season}.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                fig.savefig(
                    f"{save_path}/ACF_PACF_Emissions_{season}.svg",
                    dpi=300,
                    bbox_inches="tight",
                )
            plt.show()
    ...
    def run_uniroot_tests(self, y_col="hourly_emissions", group_col="year"):
        ...
        for group in groups:
            ...
            model = ols(
                f"{y_col} ~ factorinterval * factormonth + factordow + trend",
                data=subset,
            ).fit()
    ...
    def run_msarx_regime_test_single_year(
        self,
        target_year=2019,
        y_col="hourly_emissions_res",
        exog_cols=["hourly_generation_nonrenewables_res"],
        p=1,
        N=99,
    ):
        """
        Formally tests H0: Linear AR(X) vs H1: MS-AR(X) for a SINGLE YEAR
        using the Local Monte Carlo Likelihood Ratio Test (Rodriguez-Rondon & Dufour, 2025).
        """
        import multiprocessing
        import numpy as np
        import pandas as pd
        ...
        # ── Load R packages ───────────────────────────────────────────────────────
        try:
            MSTest = importr("MSTest")
            doParallel = importr("doParallel")
            foreach = importr("foreach")
        except Exception as e:
            print(f"⚠️ Failed to load R packages: {e}")
            return None

        df = self.data.copy()

        if target_year is not None and "year" in df.columns:
            # ── 1. Filter data ───────────────────────────────────────────────────────
            df = df[df["year"] == target_year]

        df = df.dropna(subset=[y_col] + exog_cols).reset_index(drop=True)

        if len(df) == 0:
            print("⚠️ No data available after filtering. Aborting test.")
            return None

        # ── 2. Standardise Y — keep (n, 1), R needs a (T x q) matrix ────────────
        scaler_y = StandardScaler()
        Y_np = scaler_y.fit_transform(df[[y_col]]).astype(float)  # shape (n, 1)

        # ── 3. Standardise Z ─────────────────────────
        scaler_x = StandardScaler()
        Z_np = scaler_x.fit_transform(df[exog_cols]).astype(float)

        # ── 4. Determine number of parallel workers ───────────────────────────────
        n_workers = max(1, multiprocessing.cpu_count() - 1)
        print(
            f"   Parallel workers: {n_workers} (of {multiprocessing.cpu_count()} available cores)"
        )

        # ── 5. Build fully-specified control list ─────────────────────────────────
        control_list = ListVector(
            {
                "N": int(N),
                "workers": int(n_workers),  # ← enables parallel MC simulation in MSTest
                "CI_union": False,
                "mdl_h0_control": ListVector({"const": True, "getSE": True}),
                "mdl_h1_control": ListVector(
                    {
                        "msmu": True,  # mean switches by regime
                        "msvar": False,  # variance does NOT switch
                        "getSE": True,
                        "method": "EM",
                        "maxit": 500,
                        "use_diff_init": 1,  # Use 1 for stability; higher numbers exponentially increase compute time
                    }
                ),
            }
        )

        # ── 6. Register R parallel backend ───────────────────────────────────────
        try:
            ro.r(f"doParallel::registerDoParallel({n_workers})")
            print(f"   R parallel backend registered ({n_workers} workers).")
        except Exception as e:
            print(
                f"   ⚠️ Could not register R parallel backend: {e}. Running sequentially."
            )
        ...
        try:
            with localconverter(numpy2ri_converter):
                ...
                # Convert numpy arrays → R matrix objects
                Y_r = ro.r["matrix"](
                    ro.FloatVector(Y_np.flatten()), nrow=int(Y_np.shape[0]), ncol=1
                )

                Z_r = ro.r["matrix"](
                    ro.FloatVector(Z_np.flatten(order="F")),  # column-major for R
                    nrow=int(Z_np.shape[0]),
                    ncol=int(Z_np.shape[1]),
                )

                print(
                    f"🔹 Running Local Monte Carlo LRT (N={N} simulations, p={p} lags)..."
                )

                # ── 7. Run LMCLRTest using EXPLICIT Keyword Arguments ─────────────
                # This prevents R from mixing up Z and control
                r_result = MSTest.LMCLRTest(
                    Y=Y_r, p=int(p), k0=1, k1=2, Z=Z_r, control=control_list
                )

            # ── 8. Extract results ────────────────────────────────────────────────
            available_keys = list(r_result.names)

            lr_key = next(
                (k for k in available_keys if "LRT" in k.upper() or "LR0" in k.upper()),
                None,
            )
            pval_key = next(
                (
                    ...
                    if "pval" in k.lower() or "p_val" in k.lower()
                ),
                None,
            )

            if lr_key is None or pval_key is None:
                print(f"⚠️ Could not find result keys. Available: {available_keys}")
                return None

            test_stat = float(r_result.rx2(lr_key)[0])
            p_value = float(r_result.rx2(pval_key)[0])

            # Guard against degenerate boundary fits (p11 → 0 or 1)
            if np.isnan(test_stat) or np.isnan(p_value):
                print("⚠️ NaN result — likely a degenerate/boundary model fit.")
                return None

            # ── 9. Report results ─────────────────────────────────────────────────
            print(f"\n✅ MC-LRT Regime Test Complete for {target_year}")
            print("-" * 50)
            print(f"  Observed LR Statistic : {test_stat:.4f}")
            print(f"  Monte Carlo p-value   : {p_value:.4f}  (N={N} simulations)")
            print(f"  Test specification    : mean switching only")
            print(f"  Exogenous variables   : {exog_cols}")
            print(f"  Parallel workers      : {n_workers}")
            print("-" * 50)

            if p_value < 0.05:
                print("➡️  Reject H0. A 2-regime MS model is statistically justified.")
            else:
                print(
                    "➡️  Fail to reject H0. Insufficient evidence for regime switching."
                )

            return {
                "Year": target_year,
                "LR_Statistic": test_stat,
                "p_value": p_value,
                "N_obs": len(df),
                "N_sim": N,
                "lags": p,
            ...
            # Always release the R parallel cluster, even if the test fails
            try:
                ro.r("doParallel::stopImplicitCluster()")
                print("   R parallel backend released.")
            except Exception:
                pass
    ...
    def compare_models_standardized(self):
        ...
        exog_cols_raw = [
            "hourly_generation_renewables_res",
            "hourly_generation_nonrenewables_res",
        ]
        ...
        try:
            ...
            arima_mod = ARIMA(endog=y_scaled, exog=X_scaled, order=final_order)
            arima_res = arima_mod.fit()

            results_list.append(
                get_model_stats(f"ARIMA {final_order} (Standardized)", arima_res)
            )
    ...
    def mef_estimate(self, group_col="year", include_markov=True):
        ...
        for group in groups:
            ...
            diff_model = OLS(
                diff_df["d_em"], add_constant(diff_df[["d_ren", "d_nonren"]])
            ).fit()
            ...
            try:
                ...
                arima_exog = subset[
                    [
                        "hourly_generation_renewables_mkwh_res",
                        "hourly_generation_nonrenewables_mkwh_res",
                    ]
                ]

                arima_model = ARIMA(
                    endog=subset["em_res"], exog=arima_exog, order=final_order
                ).fit()
            ...
            avg_em = subset["hourly_emissions_mlb"].sum() / (
                subset["hourly_generation_renewables_mkwh"].sum()
                + subset["hourly_generation_nonrenewables_mkwh"].sum()
            )
    ...
    def plot_annual_diagnostics_MSM(self, lags=100, save_path="images_msm"):
        ...
        for year in years:
            ...
            plt.tight_layout()
            if save_path:
                fig4.savefig(f"{save_path}/MSM_Res_{year}.pdf", bbox_inches="tight")
            if save_path:
                fig4.savefig(f"{save_path}/MSM_Res_{year}.svg", bbox_inches="tight")
            plt.show()

            # Plot 2: ACF
            fig5, ax5 = plt.subplots(figsize=sq_size)
            plot_acf(
                df_plot["Residuals"].dropna(),
                lags=lags,
                ax=ax5,
                color="#1f77b4",
                zero=False,
                title=f"ACF MSM ({year})",
            )
            ax5.set_ylim(-1.05, 1.05)
            ax5.grid(True, alpha=0.3)
            plt.tight_layout()
            if save_path:
                fig5.savefig(f"{save_path}/MSM_ACF_{year}.pdf", bbox_inches="tight")
            if save_path:
                fig5.savefig(f"{save_path}/MSM_ACF_{year}.svg", bbox_inches="tight")
            plt.show()
            ...
            plot_pacf(
                ...
                zero=False,
                method="ywm",
                title=f"PACF MSM ({year})",
            )
            ax6.set_ylim(-1.05, 1.05)
            ax6.grid(True, alpha=0.3)
            plt.tight_layout()
            if save_path:
                fig6.savefig(f"{save_path}/MSM_PACF_{year}.pdf", bbox_inches="tight")
            if save_path:
                fig6.savefig(f"{save_path}/MSM_PACF_{year}.svg", bbox_inches="tight")
            plt.show()
    ...
    def plot_smoothed_probabilities(self, lags=1):
        ...
        X_unscaled = subset[
            ["hourly_generation_renewables_res", "hourly_generation_nonrenewables_res"]
        ]

        X_scaled_array = scaler_x.fit_transform(
            X_unscaled
        )  # C. Create exog WITHOUT manual constant
        ...
        print(
            f"⏳ Fitting MSM on full time series ({len(subset)} observations)... this may take a moment."
        )
        ms_model = MarkovAutoregression(
            endog=y_scaled,
            exog=X_scaled,
        ...
        smoothed_prob_0, smoothed_prob_1 = zip(
        ...
        subset = subset.iloc[1:, :].reset_index(
            drop=True
        )  # Adjust for initial missing due to AR(1)
        # 5. Plotting
        self.set_publication_style()
        ...
        axes[0].set_title(
            f"A. Hourly Emissions Colored by Regime (Full Series)",
            loc="left",
            weight="bold",
        )
        axes[0].plot(
            subset["date_hour"],
            subset["hourly_emissions_res"],
            color="gray",
            alpha=0.3,
            lw=0.3,
            label="Observed",
        ...
        axes[0].scatter(
            subset.loc[low_mask, "date_hour"],
            subset.loc[low_mask, "hourly_emissions_res"],
            color="#1f77b4",
            s=0.5,
            alpha=0.5,
            label="Low MEF Regime",
        )
        ...
        axes[1].set_title(
            "B. Smoothed Probability of High MEF Regime", loc="left", weight="bold"
        )
        # Plot area chart
        axes[1].fill_between(
            subset["date_hour"],
            0,
            smoothed_probs[high_regime],
            color="#d62728",
            alpha=0.3,
        )
        axes[1].plot(
            subset["date_hour"], smoothed_probs[high_regime], color="#d62728", lw=0.5
        )
        ...
        axes[2].set_title(
            "C. Viterbi Path (Most Likely Regime Sequence)", loc="left", weight="bold"
        )
        ...
        axes[2].step(
            subset["date_hour"], binary_path, where="mid", color="black", lw=0.8
        )
    ...
    def plot_mef_by_year(self, results_df, include_markov=True, hawkes=True):
        ...
        if include_markov and "MS_High_MEF" in results_df.columns:
            ...
            add_trend(
                ax1,
                years,
                results_df["MS_High_MEF"],
                results_df["MS_High_SE"],
                "MS-HIGH",
                "blue",
                "d",
            )
            ...
            add_trend(
                ax1,
                years,
                results_df["MS_Low_MEF"],
                results_df["MS_Low_SE"],
                "MS-LOW",
                "purple",
                "v",
            )
        ...
        if hawkes and "Diff_MEF_2" in results_df.columns:
            add_trend(
                ax2,
                years,
                results_df["Diff_MEF_2"],
                results_df["Diff_SE_2"],
                "HAWKES",
                "gray",
                "s",
            )
